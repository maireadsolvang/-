...
ir_receive_threshold = now - datetime.timedelta(seconds = 10) #only gets action 6 (ir send) and starts a new game if within this threshold
...
def request_handler(request):
  ...
  ##################################
    ...
    try:
        ...
        #GAME TABLE
        c.execute('''CREATE TABLE game_table (game_ID integer primary key autoincrement, player_1 text, health_1 integer, player_2 text, health_2 integer, game_state int);''')
	## if already created, move to except
      ...
      if action == 0: #CORRELATION
        ...
        ##############    Actions ##################
          ...
          ##SHANA DONE##
          c.execute('''SELECT player_ID FROM gestures_table WHERE (checked = 0 AND action = 6 AND player_ID != ? AND timing > ?);''', (player_ID,ir_receive_threshold))
          results = c.fetchone()
          if results is None:
            add_action(game_ID=-1,player_ID,action)
          else:
...
def update_game_info(game_ID,player_1,health_1,player_2,health_2,game_state):
  ...
  Returns game state.
  """
  c.execute('''UPDATE game_table SET health_1 = ?, health_2 = ?, game_state = ? WHERE game_ID = ?;''',(health_1,health_2,game_state,game_ID))
  return state
...
def extract_player_gestures(player_1,player_2):
  ...
  if player_1 == None and player_2 == None:
  	return {recent_player_1['player_ID']: None, recent_player_2['player_ID']: None}
  elif player_1 == None:
  	return {recent_player_1['player_ID']: None, recent_player_2['player_ID']: recent_player_2['action']}
  elif player_2 == None:
    return {recent_player_1['player_ID']: recent_player_1['action'], recent_player_2['player_ID']: None}
	else:
    {recent_player_1['player_ID']: recent_player_1['action'], recent_player_2['player_ID']: recent_player_2['action']}
...
def check_game_start():
  ...
  """
  last_6_time = c.execute('''SELECT timing FROM game_table WHERE (action = 6 AND checked = 0) ORDER BY timing DESC LIMIT 1;''').fetchone()['timing']
  last_7_time = c.execute('''SELECT timing FROM game_table WHERE (action = 7 AND checked = 0) ORDER BY timing DESC LIMIT 1;''').fetchone()['timing']
