...
void loop(){
  ...
  Serial.println("TIM: " + String(millis()-power_timer));
  if(Power == true){
    loop_on();
  }
  else{
...
//  //POWER
  ...
  Serial.println("action_state: " + String(action_state));
  Serial.println(String(health) + " " + String(game_state) + " " + String(game_ID));
  Serial.println("current action " + current_action);
  /// speaker //////
  ...
  //Serial.print(" / ");
  //Serial.print("health: "+String(health));
  ...
  switch (action_state) {
    ...
    case GAME_RESTING: {
        ...
        data_timer = 0;
        game_state = 0;
        /////BUTTON PRESS TO SEND IR SIGNAL//////
        int button_reading = !digitalRead(button_pin);
        //Serial.println("button: " +String(button_reading));
        action_state = GAME_RESTING;
        if (button_reading) {
          do_POST(7);
          Serial.println("PRESSSSED");
    ...
    case WAITING_TO_START: {
        ...
        if (game_state == 1) { //GAME STARTED
          action_state = IN_GAME;
//          myDFPlayer.play(SONG_FIGHT);
        }
        else if (!button_reading) {
          action_state = GAME_RESTING;
          //GAME HAS BEGUN ->LEDS AND SOUND EFFECTS
        }
        else{
          do_GET(1);
          Serial.println("do get 1");
        }
        break;
      }
    /////////////////////////////////////////////////////////////////////////////////////////////////
    case IN_GAME: {
      // Set LED TO READY FOR CHARGE //
...
void update_current_act() {
  ...
  update_charged();
//  Serial.println(String(corr_val2_x) + "," + String(corr_val2_y) + "," + String(corr_val2_z));

//  Serial.println("corr_val " + String(corr_val2));
  if (magnetometer()) {
