    ...
    if method == "POST":
        ...
        ## if already created, move to except
            ...
            if action == 0: #CORRELATION
                ...
                ################# GET REQUEST ####################################################
                if action == 1: ## error Get request
                    return "ERROR: this should be a GET request! DONT POST IT"
                ...
                ##############    Actions #################################################################
                    ...
                    ### terminate early and return values if game is nto ongoing #####
                    if game_state != 1:
                ...
                #########################################################################
                    ...
                    # do a post request with game ID as null
                    add_action(NULL,player_ID,action_ID,now,0)
                    return str(5) + "," + str(2) + "," + str(0)
                ...
                ###########################################################################
                    ...
                    # do a post request with game ID as null
                    add_action(NULL,player_ID,action_ID,now,0)
                    game_info = start_game(player_ID)
                    return str(5) + "," + str(game_info[0]) + "," + str(1)
                    # FRANKLIN

    elif method == "GET":
        pass
        """
        keeps checking the gestures database to see if received signal and game has started.  USE checked_game_start()  helper fucntion
        """
    ## SHANA ##
    
def add_action(game_ID,player_ID,action_ID,time,check=0):
    #DONE
    """
    ...
    returns nothing.
    """
    c.execute('''INSERT into gestures_table VALUES (?,?,?,?,?);''',(game_ID,player_ID,action_ID,time,check))
...
def update_game_info(game_ID,player_1,health_1,player_2,health_2,game_state):
    ...
    Returns game state.
    """
    c.execute('''UPDATE game_table SET player_1 = ?, health_1 = ?, player_2 = ?, health_2 = ?, game_state = ? WHERE game_ID = ?;''',(player_1,health_1,player_2,health_2,game_state))
    return game_state
...
def start_game(receiver_ID,start_health=5,game_state=1):
    #BOOF
    """
    finds the game_initiater's name, then creates a new game using his name
    Goes into game table and finds the last entry. Returns a list of that game_ID 
    and the other player's name.
    """
    # get sender_ID
    sender_ID = c.execute('''SELECT player_ID FROM game_table WHERE (action = 6 AND checked = 0) ORDER BY timing ASC LIMIT 1;''').fetchone()
    # set that row's checked = 0
    c.execute('''UPDATE game_table SET checked = 1 WHERE (action = 6 AND checked = 0 AND player_ID = ?);'''(sender_ID)).fetchone()
    # create new game row
    c.execute(''' INSERT into game_table VALUES (?,?,?,?,?);''',(sender_ID,start_health,receiver_ID,start_health,1))
    # extract the gmame_ID
    last_entry = c.execute('''SELECT * FROM game_table ORDER BY timing ASC;''').fetchone()
    game_ID = last_entry['game_ID']
    if last_entry['player_1'] == receiver_ID:
